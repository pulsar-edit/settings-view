    ...
    this.packageManager.loadCompatiblePackageVersion(this.pack.name, (err, pack) => {
      ...
      if (packageVersion) {
        ...
        if (packageVersion !== this.pack.version) {
          ...
          this.refs.packageMessage.textContent = `Version ${packageVersion} is not the latest version available for this package, but it's the latest that is compatible with your version of ${atom.branding.name}.`
        }
        ...
        this.refs.packageMessage.insertAdjacentText(
          'beforeend',
          `There's no version of this package that is compatible with your ${atom.branding.name} version. The version must satisfy ${this.pack.engines.atom}.`
        )
        console.error(`No available version compatible with the installed ${atom.branding.name} version: ${atom.getVersion()}`)
      }
  ...
  handleButtonEvents (options) {
    ...
    const updateButtonClickHandler = (event) => {
      ...
      this.update().then(() => {
        ...
        const notification = atom.notifications.addSuccess(`Restart ${atom.branding.name} to complete the update of \`${this.pack.name}\`.`, {
          dismissable: true,
          buttons: [{
            text: 'Restart now',
